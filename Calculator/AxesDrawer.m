...
+ (void)drawHashMarksInRect:(CGRect)bounds originAtPoint:(CGPoint)axisOrigin scale:(CGFloat)pointsPerUnit
{
	...
	if (boundsContainsOrigin) {
        ...
        BOOL xOutside = axisOrigin.x >= (bounds.origin.x + bounds.size.width)
                    ||  axisOrigin.x < (bounds.origin.x);
        BOOL yOutside = axisOrigin.y >= (bounds.origin.y + bounds.size.height)
                    ||  axisOrigin.y < (bounds.origin.y);
        // both axes are outside our bounds, nothing to draw
